...
import { getFirstPost, getFollowerPost } from '../../services/post/postService';
...
export default function FollowedResearchers() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const size = 5;
      try {
        const res = await getFollowerPost(token, page, size);
        console.log(res)
        if (res.length === 0) {
